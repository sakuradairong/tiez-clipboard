...
- 设置打开时默认展开「通用」与「外观」分组，降低首屏全折叠摩擦；剪贴板分组增加简短说明，并指向下方「高级设置」中的清理/忽略规则。
- 空状态补充「单击纯文本 / 右键带格式」粘贴提示；紧凑模式下选中或键盘聚焦行时显示操作按钮（不再仅依赖悬停）。
- 设置分组头可键盘聚焦并暴露 `aria-expanded`；主题/语言选项增加选中语义；图标按钮补充 `:focus-visible` 焦点环。
- 统一部分硬编码文案：顶栏文件传输、标签按钮、列表加载 footer、敏感图遮罩标签。
