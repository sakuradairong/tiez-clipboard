...
impl RelayError {
    ...
    fn Validation(message: String) -> Self {
        Self::new(RelayErrorKind::Validation, message)
    }

    #[allow(non_snake_case)]
    fn Network(message: String) -> Self {
        Self::new(RelayErrorKind::Network, message)
    }

    #[allow(non_snake_case)]
    fn Encryption(message: String) -> Self {
        Self::new(RelayErrorKind::Encryption, message)
    }
...
impl From<rusqlite::Error> for RelayError {
    fn from(error: rusqlite::Error) -> Self {
        Self::new(RelayErrorKind::Storage, error.to_string())
    }
}

pub trait RelayReceiptStore {
    fn prune(&self, now: i64) -> AppResult<()>;
    fn state(&self, message_id: &str) -> AppResult<Option<String>>;
    fn reserve(&self, message_id: &str, expires_at: i64, now: i64) -> AppResult<bool>;
    fn remove_reserved(&self, message_id: &str) -> AppResult<()>;
    fn persist_pending_ack(&self, message_id: &str, ack_json: &str, now: i64) -> AppResult<()>;
    fn mark_acked(&self, message_id: &str, now: i64) -> AppResult<()>;
    fn pending_ack_json(&self, message_id: &str) -> AppResult<Option<String>>;
}

pub struct SqliteRelayReceiptStore {
    database_path: std::path::PathBuf,
}

impl SqliteRelayReceiptStore {
    ...
    fn open(&self) -> AppResult<rusqlite::Connection> {
        rusqlite::Connection::open(&self.database_path).map_err(RelayError::from)
    }
}

impl RelayReceiptStore for SqliteRelayReceiptStore {
    fn prune(&self, now: i64) -> AppResult<()> {
        self.open()?.execute(
    ...
    fn reserve(&self, message_id: &str, expires_at: i64, now: i64) -> AppResult<bool> {
        let inserted = self.open()?.execute(
            ...
            rusqlite::params![message_id, expires_at, now],
        )?;
        Ok(inserted == 1)
    }
    ...
    fn persist_pending_ack(&self, message_id: &str, ack_json: &str, now: i64) -> AppResult<()> {
        self.open()?.execute(
            "UPDATE clipboard_relay_receipts
             ...
             WHERE message_id = ?1",
            rusqlite::params![message_id, ack_json, now],
        )?;
        Ok(())
    }
...
impl RelayConfig {
    pub fn new(
        ...
        let webdav_url = validate_relay_url(&webdav_url.into())?;
        let webdav_base_path = normalize_base_path(&webdav_base_path.into())?;
...
fn prune_receipts(store: &impl RelayReceiptStore, now: i64) -> AppResult<()> {
    store.prune(now)
}

fn receipt_state(store: &impl RelayReceiptStore, message_id: &str) -> AppResult<Option<String>> {
    store.state(message_id)
}

fn reserve_receipt(
    store: &impl RelayReceiptStore,
    message: &RelayTextMessageV1,
    now: i64,
) -> AppResult<bool> {
    store.reserve(&message.message_id, message.expires_at, now)
}

fn remove_reserved_receipt(store: &impl RelayReceiptStore, message_id: &str) -> AppResult<()> {
    store.remove_reserved(message_id)
}
...
fn persist_pending_ack(
    ...
    let ack_json = serde_json::to_string(ack)
        .map_err(|error| AppError::Internal(format!("serialize relay ack: {}", error)))?;
    store.persist_pending_ack(message_id, &ack_json, now)
}

fn mark_receipt_acked(store: &impl RelayReceiptStore, message_id: &str, now: i64) -> AppResult<()> {
    store.mark_acked(message_id, now)
}

fn pending_ack(store: &impl RelayReceiptStore, message_id: &str) -> AppResult<Option<RelayAckV1>> {
    store
        .pending_ack_json(message_id)?
        .map(|ack_json| {
...
pub async fn send_text(config: &RelayConfig, text: &str) -> AppResult<RelaySendResult> {
...
pub async fn fetch_latest_to_clipboard<S, F, Fut>(
    config: &RelayConfig,
    store: &S,
    set_clipboard: F,
) -> AppResult<RelayFetchResult>
where
    S: RelayReceiptStore,
    F: FnOnce(String) -> Fut,
    Fut: Future<Output = AppResult<()>>,
{
    ...
    for message_ref in message_refs {
        ...
        if let Some(ack) = pending_ack(store, &message_ref.message_id)? {
            if create_json_resource(&client, config, &ack_path, &ack)
                .await
                .is_ok()
            {
                mark_receipt_acked(store, &message_ref.message_id, now_ms())?;
                continue;
            }
        ...
        if receipt_state(store, &message_ref.message_id)?.is_some() {
            continue;
        }
        ...
        if !reserve_receipt(store, &message, now)? {
            continue;
        }
        if let Err(error) = set_clipboard(content).await {
            remove_reserved_receipt(store, &message.message_id)?;
            return Err(error);
        }
        let ack = create_ack(
            &message.message_id,
        ...
        persist_pending_ack(store, &message.message_id, &ack, now_ms())?;
        let acked = create_json_resource(&client, config, &ack_path, &ack)
            .await
            .is_ok();
        if acked {
            mark_receipt_acked(store, &message.message_id, now_ms())?;
        }
        return Ok(RelayFetchResult {
            outcome: if acked {
                "copied".to_string()
...
mod tests {
    ...
    #[test]
    fn sqlite_receipts_preserve_reserved_pending_and_acked_states() {
        let path = std::env::temp_dir().join(format!(
            "tiez-relay-receipts-{}-{}.db",
            std::process::id(),
            Uuid::new_v4()
        ));
        ...
        connection
            .execute_batch(
                "CREATE TABLE clipboard_relay_receipts (
        ...
        let store = SqliteRelayReceiptStore::new(&path);
        assert!(store.reserve("message-1", 5_000, 1_000).unwrap());
        assert!(!store.reserve("message-1", 5_000, 1_001).unwrap());
        assert_eq!(
            store.state("message-1").unwrap().as_deref(),
            Some("reserved")
        );
        store
            .persist_pending_ack("message-1", r#"{"schema_version":1}"#, 1_100)
            .unwrap();
        assert_eq!(
            store.pending_ack_json("message-1").unwrap().as_deref(),
            Some(r#"{"schema_version":1}"#)
        );
        store.mark_acked("message-1", 1_200).unwrap();
        assert_eq!(store.state("message-1").unwrap().as_deref(), Some("acked"));
        assert!(store.pending_ack_json("message-1").unwrap().is_none());
        store.prune(5_000).unwrap();
        assert!(store.state("message-1").unwrap().is_none());
