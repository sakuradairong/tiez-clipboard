use crate::content_identity::{calc_text_hash, uses_text_content_hash};
use rusqlite::{params, Connection, Result};

const ENCRYPT_PREFIX: &str = "dpapi:";

pub fn run_migrations(conn: &Connection) -> Result<()> {
    run_migrations_with_decrypt(conn, |_| None)
}

pub fn run_migrations_with_decrypt(
    conn: &Connection,
        ...
        let backfill = (|| -> Result<()> {
            ...
            for row in rows {
                ...
                let plain_content = if stored_content.starts_with(ENCRYPT_PREFIX) {
                    let decrypted = decrypt_value(&stored_content);
...
#[cfg(test)]
mod tests {
    use super::{has_column, run_migrations, run_migrations_with_decrypt};
    use crate::content_identity::calc_text_hash;
    ...
    fn migration_14_hashes_decrypted_sensitive_content() {
        let conn = setup_version_11_hash_schema();
        let encrypted = "dpapi:migration-fixture";
        conn.execute(
            "INSERT INTO clipboard_history (id, content_type, content, content_hash)
             VALUES (1, 'text', ?1, ?2)",
        ...
        run_migrations_with_decrypt(&conn, |cipher| {
            (cipher == encrypted).then(|| "hello ".to_owned())
        })
        .expect("run migration 14");
